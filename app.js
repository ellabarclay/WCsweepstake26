...
async function init(){ setupTabs(); $("fixtureFilter").addEventListener("input", e => { state.fixtureFilter = e.target.value; renderFixtures(); }); state.participants = await loadJson("data/participants.json", PARTICIPANT_FALLBACK); state.results = await loadJson("data/results.json", {matches:[], updatedAt:null, source:"No results data loaded"}); state.cards = await loadJson("data/cards.json", {teams:{}, updatedAt:null}); render(); setInterval(async()=>{ state.results = await loadJson("data/results.json", state.results); state.cards = await loadJson("data/cards.json", state.cards); render(); }, 5*60*1000); }
init();
